...
| -------------------- | ----------------------------------- | --------------------------------------------------------------- |
| `addDesktopIcon`     | `icon: Omit<DesktopIcon, 'id'>`     | Adds a new icon at the specified `x`/`y` coordinates.           |
...
The widgets panel slides in from the left side of the desktop and is toggled with `Win + W` or the widgets button in the taskbar. It is managed by `useUI()` from `UIContext`.
