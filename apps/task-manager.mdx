...
description: "NEX OS Task Manager uses a WebAssembly engine to calculate CPU and memory metrics at near-native speed, delivering real-time performance graphs at 60 FPS."
---
...
| `calculateLoad` | `(cpu: number, mem: number) => number` | Calculates a weighted system load score |
| `getRank` | `(processName: string, cpu: number) => number` | Returns a priority rank (0–3) for a process |
| `simulateProcesses` | `(processes, tick, openWindows) => Process[]` | Runs a full process simulation tick |
| `processColor` | `(name: string) => string` | Deterministic hex color for a process name |

When the WASM module is not yet loaded or fails to initialize, `useWasmEngine` automatically falls back to a pure JavaScript implementation. The Task Manager UI shows a badge in the top-right corner indicating which engine is active:
...
export function calculateMetrics(
  ...
  // Near-native calculation
  return (cpuUsage * 0.7) + (memUsage * 0.3);
}
```

```typescript
// React component — TaskManager.tsx
const TaskManager = () => {
  const metrics = wasmModule.calculateMetrics(0.85, 0.40);
  return <div>Load: {metrics.toFixed(2)}%</div>;
};
```
